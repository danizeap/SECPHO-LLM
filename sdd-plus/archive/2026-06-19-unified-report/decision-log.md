...
| 2026-06-19 | Surface shared NEEDS + same-LOCATION in the report and make the rationale lead with them | Owner's live report ranked Francisco #1 but the report dropped his real drivers (same town Tres Cantos + shared infrastructure needs), so the LLM justified the top pick with generic hobbies ("cine, viajar") — unserious for a member deliverable. Needs/location come from the matcher row (passed through on the tuned path); prompt now leads with professional reasons and relegates hobbies to a light networking note | Recompute needs from free-text members (rejected: matcher already computes it well); leave as-is (rejected: weak #1 rationale) |
