...
Keep the single-file SECPHO intelligence app (`backend_api/mvp_web_app.py`) safe to expose publicly: admin data stays fail-closed, sessions are stable, requests degrade gracefully, and runtime config/state are robust on fresh deploys.
